...
namespace Kafka.Client.ZooKeeperIntegration.Partitioning
{
	using System;
    ...
    public class ZKBrokerPartitionInfo : IBrokerPartitionInfo, IZooKeeperStateListener
    {
        ...
        public ZKBrokerPartitionInfo(IZooKeeperClient zkclient, Action<int, string, int> callback = null)
        {
            ...
            this.brokerTopicsListener = new BrokerTopicsListener(this.zkclient, this.topicBrokerPartitions, this.brokers, callback);
		...
		/// <param name="callback">The callback invoked when new broker is added.</param>
		public ZKBrokerPartitionInfo(ZooKeeperConfiguration config, Action<int, string, int> callback)
			: this(new ZooKeeperClient(config.ZkConnect, config.ZkSessionTimeoutMs,
				ZooKeeperStringSerializer.Serializer, config.ZkConnectionTimeoutMs), callback)
		{
